...
func TestCalc(t *testing.T) {
	...
	operations := NewOperationsDefault()
	for _, tc := range cases {
		tc := tc
		t.Run(tc.name, func(t *testing.T) {
			got, _ := operations.Calc(tc.values)
			if got != tc.want {
...
func TestCalc2(t *testing.T) {
	...
	for _, test := range tests {
		result, err := operations.Calc(test.expression)
		if test.shouldFail {
			if err == nil {
...
func TestDefaultCalc(t *testing.T) {
	cases := []struct {
		name   string
		values string
		want   float64
...
func TestDefaultCalc2(t *testing.T) {
	tests := []struct {
		expression string
		expected   float64
		shouldFail bool
